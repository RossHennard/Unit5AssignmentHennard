...
int main()
{
     while (myNumberAsString != "*") 
     {
       ...
       if (checkForInteger(myNumberAsString))
       {
         ...
         cout << EnglishNumber(number) << endl;
         // if i'm interpreting the task right then what the task is asking is for me to have the number converted and printed in letters instead of numbers, so i just took the            number and applied the EnglishNumber string to it, I wanted to use toupper in a loop to capitalize the first letter but i couldn't figure it out in time :( 
         cout << endl;
